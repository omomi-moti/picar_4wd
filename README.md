...
#### ・ 環境の標準化とデータ精度の向上:
学習データと本番走行時の環境を完全に一致させるため、**地面からカメラの最高部までの高さを12cmに徹底して固定**しました。
- **狙い:** 画角（カメラが捉える範囲や角度）を一定に保つことで、モデルが「線の位置や角度」を正しく認識できるようにしました。
- **効果:** 異なるタイミングでデータを収集しても常に同じ条件の画像が得られ、誤認識の少ない非常に精度の高いモデルを作成することができました。
## データセット戦略とモデル性能 (Dataset Strategy & Performance)
#### データセットの構成
データセット構成 (Dataset Composition)
データセットは以下の枚数で構成されています。

学習用 (Train):

right: 166枚, straight: 156枚

評価用 (Validation):

right: 48枚, straight: 110枚

テスト用 (Test):

right: 258枚, straight: 166枚
